...
电荷收敛是在电荷自洽计算过程中，总能不再明显变化。实现上，判据是pw.x输出中的estimated scf accuracy小于给定值`etot_conv_thr`。
...
方法六：
真的需要那么大的真空尺寸吗，在平板模型中（slab），真空和平板越长，收敛遇到的问题就越多。检查过表面能的收敛了吗（而不只是总能的收敛）？请从短的平板做起，逐渐增长。
...
使用默认的```ion_dynamics='bfgs', cell_dynamics='bfgs'```，对于超胞只优化ion，即calculation='relax'。
