...
收敛问题一方面是物理的原因，计算不符合实际。（1）建模型不符合实际：输入的原子坐标、元素种类出错；实际中不存在的结构，在计算时会产生与原子初始电荷偏离较多的电荷分布，增加了收敛的难度；对称性的计算设置。
（2）使用的近似不符合实际，具有磁性，强的自旋轨道耦合，局域d、f轨道的过渡金属（Hubbard U或Hybrid functional），分子晶体等（范德瓦尔斯力近似），表面slab模型的偶极电场，可能需要相应的方法才能收敛以及正确地刻画物理体系。

另一方面是数值计算的原因，包括矩阵对角化出错，优化算法出错，物理上不符合实际也是一部分数值计算出错的根源。电荷收敛依赖于合理的初始原子位置、电荷混合参数、性能良好的赝势、合适的交换关联泛函。实现收敛的原则是建立的模型、使用的近似符合实际，同时适当减少冗余。如果计算中出现电荷不收敛，首先要检查的是结构是否合理（例如，两个位置重合的原子，晶格常数的单位，甚至错误的元素种类）。
