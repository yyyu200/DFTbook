...
结构驰豫收敛的判据是相邻两个离子步总能变化足够小，默认etot_conv_thr=1.0D-4，以及各个离子受力足够小，默认forc_conv_thr=1.0D-3，对于vc-relax还有cell受压强足够小，默认press_conv_thr=0.5。默认只进行nstep个离子步（默认50），在使用bfgs优化结构时，离子步scf收敛阈值conv_thr还会逐渐减小直到conv_thr/upscale，默认减小到1/100.0。
