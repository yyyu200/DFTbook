...
以体心立方W、Pt_3O_4，面心立方NaCl，底心正交$\alpha-$FeSe为例，用[TransCell](https://github.com/yyyu200/SlabMaker)变换CELL和原子坐标，并用VESTA验证（待续）。

# 分数坐标和直角坐标的相互转换

上一节的变换矩阵推导中并没有用到晶体周期性，所以可以将直角坐标看成单位正交矢量作为格子的基矢，这时CELL基矢列矢量组成的矩阵即为单位格子到CELL的变换矩阵，而逆矩阵即为倒格子列矢量组成的矩阵(或相差一个常数系数)。
