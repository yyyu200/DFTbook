...
第二，要找到面内的最小周期性单元。先通过密勒指数的定义找到一个面内周期单元（可能非最小），以这个面内周期单位为基础，找最小单元通常用蛮力法从小到大寻找若干组，对于超胞中每一个原子，分别找到面内的最小的几个单元，在不同原子取法中，找到共同面内周期单元中最小的一个，即是要找的表面结构二维最小单元，为了实现这一算法，请关注项目[SlabMaker](https://github.com/yyyu200/SlabMaker)，目前已经具备了建slab的功能。其他的实现方法，包括用MS的建模模块进行；公开的其他来源的讨论包括文献[5]，以及引用了[5]的[pymatgen](https://pymatgen.org/)中slab[建模功能](https://pymatgen.org/pymatgen.core.surface.html)。
...
从COD下载$\alpha-Al_{2}O_{3}$的[cif文件](http://www.crystallography.net/cod/1000017.cif)，用VESTA打开，画出六方的晶胞如下：
...
inplane vectors:
u:
 [ 6.66666672e-01 -3.33333313e-01  3.00064645e-09]
v:
 [-3.33333328e-01 -3.33333313e-01  3.00064645e-09]
angle:
 84.15650034981718
P2 =  [[ 6.66666672e-01 -3.33333313e-01  3.00064645e-09]
 [-3.33333328e-01 -3.33333313e-01  3.00064645e-09]
 [ 0.00000000e+00  0.00000000e+00 -1.00000000e+00]]
reduced slab cell
 [[ 5.49521970e+00 -4.33033332e+00  0.00000000e+00]
 [-2.74760966e+00 -4.33033313e+00  0.00000000e+00]
 [ 2.47338171e-08  3.89813986e-08 -2.37898727e+01]]
```

其中变换P1是得到一个预选的cell，如下图，对预选cell加入真空，转动c沿着垂直表面方向（这一步变换矩阵略），变换P2是将cell约化到具有110面内最小二维周期单元的slab，面内基矢量的夹角是84.16°，结果正确[6]。
