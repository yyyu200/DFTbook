...
$$\vec{X'}=x'_{1}\vec{a'}+x'_{2}\vec{b'}+x'_{3}\vec{c'}$$。本节考虑有撇号和无撇号的基矢选择下基矢和坐标的变换关系。周期性单元CELL的变换在数学上称为仿射变换（affine transformation），可以分解为线性部分和平移两个部分。
...
这里以原胞转换为晶胞为例，反之很容易同理得到。已知原胞的三个基矢量，以及各个原子的分数坐标。晶体中原子的分数坐标，是指以周期性单元的基矢量$\vec{v_{1}},\vec{v_{2}},\vec{v_{3}}$写出的坐标$\vec{X}=(x_{1},x_{2},x_{3})=x_{1}\vec{v_{1}}+x_{2}\vec{v_{2}}+x_{3}\vec{v_{3}}$。
