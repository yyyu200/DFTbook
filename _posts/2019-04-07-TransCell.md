...
首先，定义一般的坐标变换。按照文献[1]的约定，将坐标写为列矢量，基矢量也各自为列矢量。点X在基矢$O,\vec{a},\vec{b},\vec{c}$（$O$为原点）下的坐标$x_{1},x_{2},x_{3}$定义为
...
\quad
$$。
同一个点X在新坐标系$O',\vec{a'},\vec{b'},\vec{c'}$有
$$\vec{X'}=x'_{1}\vec{a'}+x'_{2}\vec{b'}+x'_{3}\vec{c'}$$

晶体中原子的分数坐标，是指以周期性单元的基矢量$\vec{v_{1}},\vec{v_{2}},\vec{v_{3}}$写出的坐标$\vec{X}=(x_{1},x_{2},x_{3})=x_{1}\vec{v_{1}}+x_{2}\vec{v_{2}}+x_{3}\vec{v_{3}}$。


# 晶胞和原胞的相互转换

这里以原胞转换为晶胞为例，反之可以同理得到。

已知原胞的三个基矢量，以及各个原子的分数坐标直角坐标。
