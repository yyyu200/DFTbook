...
\end{pmatrix}
\quad$$。
考虑晶格静止不动，选择不同的基矢，同一个点X对新的基矢$O',\vec{a'},\vec{b'},\vec{c'}$有不同的坐标
$$\vec{X'}=x'_{1}\vec{a'}+x'_{2}\vec{b'}+x'_{3}\vec{c'}$$。本节考虑有撇号和无撇号的坐标间的变换关系。变换可以分解为线性部分和平移两个部分。线性部分包括方向和长度的改变，线性部分由一个矩阵$\mathbf{P}$表示，$$(\vec{a'},\vec{b'},\vec{c'})=(\vec{a},\vec{b},\vec{c})\mathbf{P}=(\vec{a},\vec{b},\vec{c})\quad
\begin{pmatrix}
P_{11}& P_{12} &P_{13} \\
P_{21}& P_{22} &P_{23} \\
P_{31}& P_{32} &P_{33} \\
