...
晶体中原子的分数坐标，是指以周期性单元的基矢量$\vec{v_{1}},\vec{v_{2}},\vec{v_{3}}$写出的坐标$\vec{X}=(x_{1},x_{2},x_{3})=x_{1}\vec{v_{1}}+x_{2}\vec{v_{2}}+x_{3}\vec{v_{3}}$。按照文献[1]的约定，将分数坐标写为列矢量，基矢量也各自为列矢量，
