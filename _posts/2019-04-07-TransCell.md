...
换句话说，```ibrav```=1～13模式对于除三斜外的布拉伐格子的单元定义是不完备的。好在，对于布拉伐格子是1~13，但是不能写成ibrav=1~13的情况，在周期性边界条件下总是可以避免的，根据单元的平移对称性，可以将这个单元转换成符合ibrav=1～13的单元形式，单元的基矢量坐标可以通过坐标系的旋转、坐标轴的部分反演或置换满足```ibrav```=1~13相应的程序内部的CELL_PARAMETERS形式，见3.1节所述的单元之间的变换。
...
<span id = "note2">2.</span>关于alat，alat是qe内部定义的量，以bohr为单位，具有晶格常数的意义（这里晶格常数特指晶胞的第一个基矢量长度），在pw.x的输出接近开头处有` lattice parameter (alat)  = x.xxxx  a.u.`。
...
虽然没有硬性规定，但是，建议晶胞的三个基矢量按照从小到大的顺序排列，即a&le;b&le;c，因此CELL_PARAMETER第一行矢量具有晶格常数的意义。
...
(3)对于`ibrav`$\neq$0，alat=`celldm(1)`或`A`/0.529，alat具有晶胞晶格常数的意义。
