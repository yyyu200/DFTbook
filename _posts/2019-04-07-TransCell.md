...
建好超胞之后，变换CELL原子和分数坐标的方法为：统一选择CELL的第三个基矢方向作为垂直表面方向，一般地，将空间直角坐标系做旋转可以实现z轴沿着第三个基矢方向，且x轴沿第一个基矢方向，这时CELL基矢量变化为：
$$\quad
\begin{pmatrix}
...
$$\vec{X\prime}=(x\prime_{i1},x\prime_{i2},x\prime_{i3})^T=(x_{i1},x_{i2},{d_{vacuum}/2+(x_{i3}-x{min,3})\vert\vec c\vert}\over{\vert\vec c\prime\vert})$$。
