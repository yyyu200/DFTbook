...
得到部分pw.x的部分输入内容，pw.x其余的输入部分手动补全，
```
&SYSTEM
...
输入中加入```nspin = 2```。设置至少一种元素的starting_magnetization(i), i=1...ntyp值非零，scf计算中磁矩从此值逐渐减小直到收敛（通常设为1）。

不需要full relativistic赝势，支持relax计算。
...
在输入中加入```noncolin=.true., lspinorb=.true.,```。设置至少一种元素的starting_magnetization(i)值非零, i=1...ntyp。设置angle1(i),angle2(i)。
