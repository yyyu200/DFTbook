...
可以发现VESTA对CELL做了处理变成了一个下三角阵，但是这样不太容易找对称性（实际上pw.x确实没找全！），可以将其改写成INPUT_PW所具有的格式如下：
...
使用[FindPrim](http://www.densityflow.com/findprim.php)或[Unit2Prim](http://www.densityflow.com/unit2prim.php)转换为原胞，采用了[表格2](../../07/TransCell/#tab2)所述方法，可以实现所有类型（14种布拉伐格子中7种非简单格子，以及六方转菱方）的晶胞转原胞的功能，如果前面cif得到的已经是原胞或者属于简单格子，则输出结构相同。在Unit2Prim页面上传晶胞的pw.x输入文件，并指定Bravais格子类型，这里选择面心立方，即"2"，点击“提交”，得到原胞的输入文件。
