...
在周期性边界条件下，各个k点的各个能级的波函数，可以投影到以各个原子坐标为中心的s、p、d、f等球谐函数上（以及非共线和磁性计算），即为（local）projected density of states，用projwfc.x模块计算，下面以$SnO_{2}$(rutile)的投影态密度为例。
...
elem=['Sn','O']
ielem=np.array([2,4],dtype=np.int32)
iorb=np.array([3,2],dtype=np.int32)
orb=[['s','p','d'],['s','p']]
