...
'relax'：一系列scf计算，每个scf间通过力，计算离子坐标的变化，通过优化算法找到能量极小值点，对应晶体结构的理论值，relax计算时固定cell不变。
...
以下文件保存为ph.inp，运行```ph.x < ph.inp > ph.out```
...
将以下保存为matdyn.inp，运行```matdyn.x < matdyn.inp > matdyn.out```，计算声子色散，q点定义见安装包/Doc/brillouin_zones.pdf。结果与文献比较见[8]。
```
&input
  asr='simple',
  amass(1)=69.7230,
