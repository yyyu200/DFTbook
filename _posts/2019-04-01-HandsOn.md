...
首先介绍结构信息的获取，已经了解可以直接阅读1.2节。碳化硅SiC有多种结构，这里选择了比较简单的闪锌矿(Zincblende)结构。计算需要知道SiC的原胞尺寸、原子坐标和晶格常数，这些参数是从实验得到的，除了从文献获取外，这里推荐一个数据库[Crystallography Open Database](http://www.crystallography.net/cod/)（COD，如果你手头有更好的cif数据库也可以）。打开COD主页，点击左侧的“search”。
...
/
```
输出pwscf.plot_eps.dat，画图如下。TDDFT与GW-BSE计算相比，在DFT计算基础，GW-BSE计算使用格林函数，直接构造了物理过程，是计算光学性质效应的标准方法；TDDFT基于电子密度，并不那么直观，但计算量相对很小。这是两种互补的方法，原则上都是精确的。TDDFT使用的泛函十分关键，原则上需要非局域，如Hybrid functional、meta GGA、OEP等。
...
以上一章中$SnO_{2}$为例，用不加SOC做vc-relax得到晶格常数结果（可以换算得到celldm(1)=9.145886），进行SOC计算。
