...
QE计算的波函数是以平面波基组表示的，而原子轨道有更好的直观性和可解释性，是分析能带不可或缺的手段，可以将各个k点的各个能级的波函数，投影到以各个原子坐标为中心的s、p、d、f等球谐函数上（以及非共线和磁性计算），即为（local）projected density of states，用projwfc.x模块计算，下面以$SnO_{2}$(rutile)的投影态密度为例。
