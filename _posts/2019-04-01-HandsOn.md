...
#### 1.2.1 占据数、展宽的设置
对于导体，由于存在半满的能带，而只有占据的能级对于总能有贡献，费米能级附近的能级在自洽计算时可能出现相邻两次自洽迭代一次占据一次不占据，导致总能值震荡的情况，计算总能时，除了使用足够密的k点网格以外，考虑费米能级附近的能级都对总能有一定贡献，可以让计算更容易收敛，即设置占据数的展宽。
...
用PBE交换关联近似计算得到晶格常数理论值是3.1745Å。

### 2.2 结构驰豫的计算
这里的结构驰豫应该用```calculation='relax'```进行。
### 2.3 获取静电势并得到功函数
用average.x得到功函数。
