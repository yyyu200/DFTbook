...
### 2.1 建立平板（slab）模型
建立slab模型的一般方法见[这里](../../07/TransCell/#平板slab模型的建立)。首先计算面心立方铜的原胞，用```calculation='vc-relax'```优化晶格常数，并用得到的晶格常数理论值建slab模型。实验晶格常数为3.61Å。
...
计算态密度，nscf.inp输入如下，使用了53^3个k点的网格。
```
&CONTROL
...
计算得到的能带和态密度如图。
<p align="left">
    <img src="https://yyyu200.github.io/DFTbook/img/pwbanddos.png" width="200"/>
</p>
