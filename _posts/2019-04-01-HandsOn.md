...
功函数与晶向、重构、吸附等密切相关，这里考虑金属Cu的（001）表面，对于绝缘体的电离势可以通过类似方法得到。

### 2.1 建立平板（slab）模型
建立slab模型的一般方法见[这里](../../07/TransCell/#平板slab模型的建立)。首先计算面心立方铜的原胞，用```calculation='vc-relax'```优化晶格常数，并用得到的晶格常数理论值建slab模型。实验晶格常数为3.16469Å。
