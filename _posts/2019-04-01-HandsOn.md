...
```
加入真空时，CELL的面内基矢不变，删掉最顶端的一个原子，以保持slab的两个表面的对称性（在Cu的110面，不删除这个原子两个表面也是等势能的），z方向增加20A，原子分数坐标的z分量随之乘以一个系数，并在z方向平移，使slab位于CELL的中间。

得到输入文件如下，这里的结构驰豫应该用```calculation='relax'```进行。
