...
### 2.3 获取静电势并得到功函数
(1) 用pp.x得到静电势，如下文件保存为pp.inp，运行```pp.x < pp.inp > pp.out```，
...
用average.x对面内势能取平均，并对垂直表面方向势能取移动平均值，average.x输入说明见[这里](https://github.com/QEF/q-e/blob/master/PP/src/average.f90)，输入文件average.inp见如下，运行```average.x< average.inp >average.out```：
