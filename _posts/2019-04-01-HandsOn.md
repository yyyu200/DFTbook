...
epsilon.x计算能级间偶极跃迁得到介电常数，根据公式[7]：

$\epsilon_2 (\omega)= \frac{4 \pi ^2} {m^2\omega^2} \sum_{V,C} {\int_{BZ} {d^3 k  \frac {2}{(2\pi)^{3/2}}  \lvert eM_{CV}(K) \rvert ^2 \cdot \delta[E_{C}(k)-E_{V}(k)-h\omega]}}$

目前仅支持模守恒赝势。首先优化晶格常数，运行```pw.x < vc.inp > vc.out```，vc.inp如下：
```
&CONTROL
    calculation='vc-relax', disk_io='low', prefix='pwscf',
