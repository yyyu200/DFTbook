...
## 4. 声子谱的计算
计算GaAs的声子谱。QE中声子计算是通过DFPT进行的，通过加入berry phase电场计算电荷响应得到介电常数，新版本（>=6.x）也开始支持finite displacements method。参考安装包q-e-qe-6.3\PHonon\examples\ example1 （$\Gamma$点）和example2 （色散），画图这里用的是python。PHonon模块在同一目录重新计算时需要删除旧部分文件(./tmp/_ph0)，否则可能报错退出。
...
以下计算分为7个步骤，其中q2r.x,matdyn.x,lambda.x这三个模块运行时不用mpi并行。

(1) 较密k点的scf计算

输入如下，注意这一步加入了`la2F=.true.`，k点设置为16×16×16的网格，此处必须是第2步k点的整数倍。
...
计算电子声子耦合常数$\lambda$和Eliashberg谱函数$\alpha^{2} F(\omega)$。
...
/
```
得到输出文件alpha2F.dat，取最后一列展宽为0.05的画图如下。
...
计算电子声子耦合常数$\lambda$和McMillan公式中的超导转变温度$T_C$。$\mu^{∗}$是一个描述库仑屏蔽的经验常数，取值常在0.1到0.16之间，这里取值0.1。
