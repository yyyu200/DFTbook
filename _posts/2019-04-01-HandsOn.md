...
首先介绍结构信息的获取，已经了解的读者可以直接阅读1.2节。碳化硅SiC有多种结构，这里选择了比较简单的闪锌矿(Zincblende)结构。计算需要知道SiC的原胞尺寸、原子坐标和晶格常数，这些参数是从实验得到的，除了从文献获取外，这里推荐一个数据库[Crystallography Open Database](http://www.crystallography.net/cod/)（COD，如果你手头有更好的cif数据库也可以）。打开COD主页，点击左侧的“search”。
...
可以发现VESTA对CELL做了处理变成了一个下三角阵，但是这样不太容易找对称性（实际上pw.x确实没找全！），用[SlabMaker](https://github.com/yyyu200/SlabMaker)，读入sc.vasp，运行python examples/test.py，输出fcc.vasp，脚本导入了build.py。采用了[表格2](../../07/TransCell/#tab2)所述方法，可以实现所有类型（14种布拉伐格子中7种非简单格子，以及六方转菱方）的晶胞转原胞的功能，要求输入晶胞的POSCAR，并指定ibrav类型。结果如下：
...
在新版本(>=6.4)中加入了`localization_thr`参数，设置为0.004，限于模守恒赝势，对于大体系，可以减少计算量[19]。
