...
在输出ph.out中可以找到高频介电常数张量$\epsilon_{ij}(\infty)$（实验值10.86），以及玻恩有效电荷张量$Z_{ij}$$
```
...
输出的dynmat.out中可以找到$\Gamma$点声子频率如下（实验值：TO=271cm-1，LO=293cm-1）。
...
ph.x计算结束后，运行q2r.x，将动力学矩阵由q空间变换到实空间，输入如下，运行```q2r.x < q2r.inp > q2r.out```：
```
 &input
