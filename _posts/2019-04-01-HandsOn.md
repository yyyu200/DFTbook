...
即为弛豫后的晶格结构（或在输出文件中可以找到），可以看出晶格常数理论值为4.378Å。结构弛豫通过力的计算得到总能-位形曲面上的局部极小值，为了模拟实际，需要初始结构与实际情况足够接近。这里输出的density与ATOMIC_SPECIES中设置的原子质量有关。vc-relax计算包括多个自洽计算，查看自洽计算得到的总能用
