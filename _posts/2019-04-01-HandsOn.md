...
设置```occupation='smearing',smearing='gaussian'```，对于不同的材料调整degauss的值，degauss对于有带隙的材料，取1d-9，也就是接近0就可以，对于没有带隙的导体材料，degauss从小尝试到大，取值约0.01量级，一要总能要几乎不变，二要输出中的“smearing contribution TS”足够小，建议小于1 meV/atom。
...
```
其中，前三个数nk1, nk2, nk3是k点网格，由于对称性可能实际计算k点个数有约化。后三个数k1, k2, k3是设置k点网格是否含相应的0点。
