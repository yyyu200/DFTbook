...
fildyn是输出动力学矩阵文件，共有q点数+1个，后缀0是这些q点的坐标，后缀1-q点数是动力学矩阵、频率等值，$\Gamma$点还有光学介电常数、有效电荷等值。在输出ph.out中可以找到高频介电常数张量$\epsilon_{ij}(\infty)$（实验值10.86，Ref.[10]），以及玻恩有效电荷张量$Z_{ij}$（实验值2.07）：
```
