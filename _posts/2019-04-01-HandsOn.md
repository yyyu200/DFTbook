...
可以发现VESTA对CELL做了处理变成了一个下三角阵，但是这样不太容易找对称性（实际上pw.x确实没找全！），用[SlabMaker](https://github.com/yyyu200/SlabMaker)，读入sc.vasp，运行python examples/test.py，输出fcc.vasp，或使用基于SlabMaker的网页[QE工具](http://www.densityflow.com/)。采用了[表格2](../../07/TransCell/#tab2)所述方法，可以实现所有类型（14种布拉伐格子中7种非简单格子，以及六方转菱方）的晶胞转原胞的功能，要求输入晶胞的POSCAR，并指定ibrav类型。结果如下：
...
根据Bloch定理，非$\Gamma$点的波函数不具有晶格周期性，直接画图一般是做绝对值平方。其他情况：有自定义的计算需要波函数。

波函数绝对值平方可以由pp.x得到，其中设置`plot_num=7`。可选k点及能带。
...
平面波矢量表示(G空间)的波函数在'outdir'/'prefix'.save/wfc[xxx].dat(或hdf5格式，configure使用\-\-with-hdf5)，是二进制文件，读写参看Module/io_base.f90 
```
