...
等号右边第一项是具有点缺陷$X^q$的超胞的总能，第二项是不含缺陷的超胞总能，$n_i$是两个超胞第i种元素原子个数之差，$\mu_i$是第i种元素的化学势，$E_{VBM}$是价带顶能级位置，$\mu_e$是以价带顶为参考点的电子化学势，由于计算量限制超胞中掺杂浓度常常大于实际，需要加入$E_{corr}$有限尺寸修正项。
